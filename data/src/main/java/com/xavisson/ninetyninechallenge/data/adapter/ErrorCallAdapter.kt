    ...
    private open inner class BasicExecutorCallback(private val delegate: Callback<Any>) : Callback<Any> {
        override fun onResponse(call: Call<Any>, response: Response<Any>) {
            if (response.isSuccessful) {
                ...
                delegate.onFailure(call, NinetyNineServerExceptionFactory.create(HttpException(response)))
            }
        }

        override fun onFailure(call: Call<Any>, throwable: Throwable) {
            delegate.onFailure(call, NinetyNineServerExceptionFactory.create(throwable))
        }
    }
